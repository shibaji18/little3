...
class MenuViewTest(TestCase):
    ...
    def test_getall(self):
        ...
        self.assertEqual(response.data[0]["title"], "Chiecken hakka noodle")
        self.assertEqual(response.data[0]["price"], "10.00")
        self.assertEqual(response.data[0]["inventory"], 10)

        self.assertEqual(response.data[1]["title"], "American chop sui")
        self.assertEqual(response.data[1]["price"], "11.00")
        self.assertEqual(response.data[1]["inventory"], 20)

        self.assertEqual(response.data[2]["title"], "Butter burger")
        self.assertEqual(response.data[2]["price"], "12.00")
